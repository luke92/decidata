...
def process_data_1():
    ...
    xls = pd.ExcelFile(path_read+'excel_leer.xlsx')
...
def process_data_2():
    # CARGAR DATOS EXCEL (FALTA AGREGAR CSV)
    xls = pd.ExcelFile(path_read+'weather.xlsx')
...
def process_data_3():
    # CARGAR DATOS EXCEL (FALTA AGREGAR CSV)
    xls = pd.ExcelFile(path_read+'holiday.xlsx')
    df = pd.read_excel(xls, 'Hoja1')
    # TRANSFORMAR DATE EN datetime64[ns]
    df['date'] = pd.to_datetime(df['date'].astype(str), format='%Y/%m/%d')
...
master.to_csv(path_read+'master_prueba.csv')


# PREPARAR LAS ENTRADAS PARA LOS MODELOS DE ML
restaurant_test = pd.read_csv(path_read+'master_sin_outliers.csv', sep=",")
...
ax.add_line(line)
plt.title('RFR')
plt.savefig(path_save+'Grafico_1')
...
with open (path_read+'modelo_pedidos', 'wb') as f:
    pickle.dump(rfr, f)
...
restaurant_test = pd.read_csv(path_read+'master.csv')
...
# CARGAR EL DATASET
dataset = pd.read_csv(path_read+'original.csv', sep=",")
...
plt.savefig(path_save+'Grafico_2')
# SIN OUTLIERS
# ELIMINAR OUTLIERS
...
plt.title('Visualización de outliers')
plt.savefig(path_save+'Grafico_3')
...
plt.savefig(path_save+'Grafico_4')
...
df_anova1.to_csv(path_read+'df_anova1.csv')
...
plt.savefig(path_save+'Grafico_5')
...
plt.xlabel('day_week')
plt.ylabel('orders')
plt.savefig(path_save+'Grafico_6')
...
plt.savefig(path_save+'Grafico_7')
...
plt.xlabel('month')
plt.ylabel('day_week')
plt.savefig(path_save+'Grafico_8')
...
plt.xlabel('date')
plt.ylabel('orders')
plt.savefig(path_save+'Grafico_9')
